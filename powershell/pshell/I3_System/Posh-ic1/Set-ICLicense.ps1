...
#> # }}}3
  ...
  if ($AdditionalLicenses) {
    ...
    if ($AdditionalLicenses.Length -eq 1 -and $AdditionalLicenses[0] -eq "*") {
      $allAdditionalLicenses = ((Get-ICLicenseAllocations $ICSession).items | foreach { if (-not ($_.configurationId.id -match "I3_ACCESS_RECORDER" -or $_.configurationId.id -match "I3_ACCESS_CLIENT")) { $_.configurationId } })
      # Add missing licenses
      $allAdditionalLicenses += New-ICConfigurationId "I3_ACCESS_RECORDER"
      $allAdditionalLicenses += New-ICConfigurationId "I3_ACCESS_CLIENT"
      # $allAdditionalLicenses += New-ICConfigurationId "I3_LICENSE_BASIC_STATION"
