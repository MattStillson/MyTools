...
test(function parse() {
  ...
  (function testWithCallback() {
    ...
    form.parse(REQ, gently.expect(function parseCbOk(err, fields, files) {
      assert.deepEqual(fields, {field1: 'bbb', field2: 'nice'});
    ...
    form.parse(REQ, gently.expect(function parseCbErr(err, fields, files) {
      assert.strictEqual(err, ERR);
      assert.deepEqual(fields, {aaa: 'bbb'});
    }));
  })();
});
...
test(function _initMultipart() {
  ...
  (function testRegularField() {
    ...
    PARSER.onHeaderEnd();
    PARSER.onHeaderField(new Buffer('aaa'), 0, 3);
    PARSER.onHeaderValue(new Buffer('bbb'), 0, 3);
    PARSER.onHeaderEnd();
    PARSER.onHeadersEnd();
